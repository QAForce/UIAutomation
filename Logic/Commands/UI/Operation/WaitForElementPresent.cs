...
{
    public class WaitForElementPresent : Command
    {
        public WaitForElementPresent(String id, String description, KeyValuePair<String, Tuple<Type, String>> output, Dictionary<String, Tuple<Type, String>> parameters)
        ...
        public override void Run(TestContainer container)
        {
            try
            {
                ...
                for (int second = 0; ; second++)
                {
                    ...
                    try
                    {
                        if (IsElementPresent(by, container)) break;
                    }
                    catch (Exception)
                    {
                    }
