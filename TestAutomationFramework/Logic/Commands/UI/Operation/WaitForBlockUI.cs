...
{
    public class WaitForBlockUI :Command
    {
        public WaitForBlockUI(String id, String description, KeyValuePair<String, Tuple<Type, String>> output, Dictionary<String, Tuple<Type, String>> parameters)
        ...
        public override void Run(TestContainer container)
        {
            try
            {
                ...
                cmdHelp.ExtractMethodName("xpath=//span[contains(@class,'blockUI')]", out invokeMethodName, out invokeParameter);
                //cmdHelp.ExtractMethodName("css=blockUI", out invokeMethodName, out invokeParameter);
                ...
                for (int second = 0; ; second++)
                {
                    ...
                    try
                    {
                        if (IsElementPresent(by, container) == false) break;
                    }
                    catch (Exception)
                    {
                    }
